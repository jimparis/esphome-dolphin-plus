...
- **Response Layout (declared 256-byte frame payload, including ACK)**:
  ...
  - **Verified `leds` byte**: Live testing of the reference unit across different modes and intensities verified that data byte 155 contains the correct packed LED configuration (e.g. `0xA3` for 100% Disco, `0xA2` for 100% Constant, and `0x42` for 40% Constant). Data bytes 156 and 157 remain `0xFF` (unused).
  - **Write Command Behavior**: LED writes (`set_robot_leds`, `FFF7/10`) and steering writes (`manual_drive`, `FFF7/03`) take effect physically but do not return a clean ACK response on this unit (often causing ACL link-layer aborts or timeouts if waited for). They are now treated as write-only commands that do not wait for responses (`expects_response = false`), ensuring instant execution and preventing command queue blockage.
